...
public:
	...
	void fit(std::vector<cv::Point> input)
	{
	...
	std::vector<int> regionQuery(std::vector<cv::Point> input, int i)
	{
	...
	void expandCluster(std::vector<cv::Point> input, int i, std::vector<int> neighbours, const int cluster)
	{
		labels[i] = cluster;
